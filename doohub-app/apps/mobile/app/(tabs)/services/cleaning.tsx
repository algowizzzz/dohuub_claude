...
export default function CleaningServicesScreen() {
  ...
  const fetchListings = async () => {
    try {
      ...
      const response: any = await api.get(ENDPOINTS.SERVICES.CLEANING, params);
  ...
  const renderListingCard = ({ item }: { item: any }) => (
    <Card style={styles.listingCard} onPress={() => handleListingPress(item)}>
      <View style={styles.listingImage}>
        <Ionicons name="sparkles" size={40} color={colors.text.muted} />
      </View>
      ...
      <View style={styles.listingContent}>
        ...
        <View style={styles.listingMeta}>
          <Badge 
            text={CLEANING_TYPES.find(t => t.id === item.cleaningType)?.label || item.cleaningType}
            variant="primary"
            size="sm"
          />
          {item.duration && (
