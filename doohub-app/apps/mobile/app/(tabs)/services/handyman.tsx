...
const HANDYMAN_TYPES = [
  ...
  { id: 'GENERAL_REPAIR', label: 'Repairs' },
];
...
export default function HandymanServicesScreen() {
  ...
  const fetchListings = async () => {
    try {
      ...
      const response: any = await api.get(ENDPOINTS.SERVICES.HANDYMAN, params);
  ...
  const renderListingCard = ({ item }: { item: any }) => (
    <Card style={styles.listingCard}>
      <View style={styles.listingImage}>
        <Ionicons name="hammer" size={40} color={colors.text.muted} />
      </View>
      ...
      <View style={styles.listingContent}>
        ...
        <View style={styles.listingMeta}>
          <Badge 
            text={HANDYMAN_TYPES.find(t => t.id === item.handymanType)?.label || item.handymanType}
            variant="primary"
            size="sm"
          />
        </View>
        ...
        <View style={styles.listingFooter}>
          ...
          <Text style={styles.price}>${item.basePrice}/{item.priceUnit?.replace('per_', '')}</Text>
        </View>
      </View>
    </Card>
  );
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            <Ionicons name="hammer-outline" size={48} color={colors.text.muted} />
            <Text style={styles.emptyText}>No handyman services available</Text>
          </View>
        }
      />
    </View>
