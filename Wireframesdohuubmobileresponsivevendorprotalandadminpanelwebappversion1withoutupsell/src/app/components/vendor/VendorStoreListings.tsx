...
export function VendorStoreListings() {
  ...
  const [store, setStore] = useState<{ name: string; category: string } | null>(null);
  ...
  useEffect(() => {
    const fetchStoreListings = async () => {
      ...
      try {
        ...
        setStore({
          name: storeData.name || storeData.businessName,
          category: storeData.category,
        });
        
        // Fetch store listings
        const listingsData: any = await api.getStoreListings(storeId);
        const mappedListings: Listing[] = (listingsData.listings || listingsData || []).map((listing: any) => ({
          id: listing.id,
          title: listing.title || listing.name,
          description: listing.description || "",
          price: listing.price || listing.basePrice || 0,
          bookings: listing.bookings || listing.bookingCount || 0,
          bookingTrend: listing.bookingTrend || 0,
          status: (listing.status || "ACTIVE").toUpperCase() as "ACTIVE" | "PAUSED" | "SUSPENDED",
          rating: listing.rating || listing.averageRating || 0,
          reviews: listing.reviews || listing.reviewCount || 0,
        ...
        console.error("Failed to fetch store listings:", err);
  ...
  // Helper functions (moved from vendorBusinessData.ts)
  const getCreateButtonText = (category: string) => {
    const productCategories = ["Groceries", "Food", "Beauty Products"];
    return productCategories.includes(category) ? "Create New Product" : "Create New Service";
  };

  const isProductCategory = (category: string) => {
    const productCategories = ["Groceries", "Food", "Beauty Products"];
        ...
        <div className="max-w-[1400px] mx-auto">
          ...
          <div className="mb-6">
            <h1 className="text-2xl sm:text-[28px] lg:text-[32px] font-bold text-[#1F2937] mb-3">
              Manage Listings: {store?.name || "Loading..."}
            </h1>

            {/* Profile Info Line */}
            <div className="flex flex-wrap items-center gap-2 text-sm text-[#6B7280] mb-6">
              <span className="hidden sm:inline">
                <span className="font-medium">Business:</span> {store?.name || "Loading..."}
              </span>
              <span className="hidden sm:inline">•</span>
              <span>
                <span className="font-medium">Category:</span>{" "}
                {store?.category || "Loading..."}
              </span>
              <span>•</span>
              <span>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              >
                {store ? getCreateButtonText(store.category) : "Create New Listing"}
              </Button>
